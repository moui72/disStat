...
fitems <- items %>% gather(key="version",value="text",-no,-asp,-v) %>% arrange(asp,v,version,text) 

fitems[c(4:5)] %>% 
  kable(col.names=c("Version", "Text"),booktab=T,linesep = c("", "", "", "\\hline")) %>% 
  collapse_rows(1) %>%
  row_spec(c(1:64), hline_after = F, extra_css = "border:0") %>%
  row_spec(seq(5,64,by=4), hline_after = T, extra_css = "border-bottom: 1px solid #999") %>%
...
names(items) <- c("no","+PP D","+PP Q","-PP D","-PP Q","asp")

fitems <- items %>% gather(key="version",value="text",-asp,-no) %>% arrange(asp,no,version) 

fitems[c(3:4)] %>% 
  kable(col.names=c("Version", "Text"),booktab=T,linesep = c("", "", "", "\\hline")) %>% 
  row_spec(c(1:64), hline_after = F, extra_css = "border:0") %>%
  row_spec(seq(3,64,by=2), hline_after = T, extra_css = "border-bottom: 1px solid #999") %>%
  kable_styling(latex_options = c("hold_position","repeat_headers"),bootstrap_options = c("hover"))
