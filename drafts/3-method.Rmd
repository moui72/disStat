...
It is worth noting that some linguists (e.g., @den2006relators) believe *of* is not always a preposition in the same sense as *from*, *on*, or *in*, etc., in that it appears in some cases to be serving a strictly functional role, without real lexical content. Importantly, it is also only 2 characters \added{long}, whereas *into*, *onto*, and *from* (the other possible heads of PP2) are all 4 characters. This is revisited and its possible impact is explored in the results section (section \@ref(pp2h)).
...
All participants in the \added{current study} were undergraduate students enrolled at CUNY's Queens College in Psychology 101[^irb] who participated for course credit. Self-reported age ranged from 18 to 25 years. Participants were recruited \added{via} a software system designed for university participant pools. Students saw a recruitment notice on the system website (see Appendix \@ref(rec)), and were able to schedule their own appointment time within a range of hours offered.
...
```{r screens, fig.cap="Diagram of 4-screen sequence presented for each item, showing the key presses triggering movement between successive screens.", fig.height=2.5}
...
It is likely, then, \added{that} an oral reader's voice would actually still be on the object when the eyes' fixation begins to provide information of some kind about the existence of PP2, and will still be pronouncing PP1 when the eyes are first fixated on PP2. This raises a question about any prosodic breaks produced after the object, because it is difficult to distinguish between an intentional prosodic break at that point, and one arising from the reader using a natural \added{position for a} break \replaced{to}{for} hesitate \replaced{due to}{related to} the garden path effect of discovering the disambiguating PP2. \added{This property of oral reading calls into question the status of OBJ breaks to be reported in Chapter} \@ref(res) \added{with regard to whether they are linguistically motivated prosodic boundaries, or represent hesitation due to the processing difficulty experienced when PP2 is first observed by the reader.}
...
To verify the accuracy of the computer measurement, timings were collected by hand for `r 120*2` recording\added{s}. There was a significant positive correlation between hand-measured and computer-measured timings (r(118)=0.87, p < 0.001), with a median difference of 0.4s (SD = 1.5)[^error].
...
\replaced{The rater}{She} was asked to report on whether or not she heard a prosodic boundary directly following the region labeled **OBJ**, and directly \deleted{after the} following \added{the region} labeled **PP1**. Note that throughout this dissertation, the word "break" is used interchangeably with the word "boundary" when referring to prosodic boundaries. It should be noted that a prosodic break represents only one kind of prosodic boundary, but the judgements elicited for this study do not distinguish between different sorts of boundaries, and it proves useful to employ the term "break" as a shortcut in some instances. \replaced{Instruction regarding how to define}{The following definition of} prosodic boundary was provided \added{by way of the following prose}:
...
The decision to use loose intuitive judgments of prosodic phenomena, rather than an automatic computer-based classification system or an established standard like ToBI, is based on (a) the fact that prosodic phenomena are difficult to phonetically describe (cf. @choi2005finding), and computer performance is typically worse than human informants, and (b) the fact that the level of detailed offered by more robust sytems is simply not necessary for the analyses pursued here.

Detailed instructions on the order in which items should be coded, both within speaker and across speakers, were also provided. The result was that she never listened to both readings of a sentence in sequence; she never listened to two Reading 1 versions of different sentences in sequence; and she never listened to the sentences in the same order for a different participans. The instructions given to the informant and the set of judgments requested can be found in Appendix \@ref(RA).
...
t<-kable(
  comps[,c(1,3:5)],
  caption="Percent agreement between informants (inter-rater) and first and second codings by the primary informant (intra-rater).", 
  booktab=T, 
  digits=2,
  align="c",
...
0 < K $\\leq$ 0.40,Unreliable distinction
...
0.80 < K $\\leq$ 1.0,Highly reliable distinction
"
