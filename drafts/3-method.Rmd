...
<!--![Procedure diagram](dissertation-procedure.svg)-->
...
Details on the instructions given and the judgements collected can be found in the apppendices (\ref{sec:RA}).

### Reliability

A second trained linguist repeated the task over 128 recordings selected from 8 participants (two from each group, one per ordering). Even number experimental items were used from 4 participants, and odd numbered from the other 4. There were 8 recordings missing from the 128 selected, so the reliability task resulted in judgements over 120 recordings. The first informat also blindly re-rated those 120, with the recording name obscured and instructions not to revisit her original ratings. Reliability scores are reported in table \ref{tab:validity}.

```{r validity}
library(irr)
dr<-read_csv("../csvs/src/DR-V.csv") %>% arrange(UID)
sp<-read_csv("../csvs/src/SP-V.csv") %>% arrange(UID)
main<-read_csv("export/prosody_data.csv") %>% subset(UID %in% dr$UID) %>% arrange(UID)


vals = c("V","OBJ","PP1","STRUG","STRONG","WEAK","STRUG_START","QI")
...
drsp<-merge(dr,sp,by="UID",suffixes=c("-dr","-sp"))
