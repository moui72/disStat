...
for(col in subset(vals, !vals %in% logicals)){
  lvls<-union(
    unique(all[[col]]),
    unique(all[[paste0(col,suffixes[1])]]))
  levels<-union(lvls,
    unique(all[[paste0(col,suffixes[2])]])
  )
  all[[col]] <- factor(all[[col]], levels=lvls)
  for(suf in suffixes){
    all[[paste0(col,suf)]] <- factor(all[[col]], levels=c("OBJ", "PP1", "PP2", "START", "SUBJ", "V", NA))
  }
}

for(val in vals) {
  for(suf in suffixes) {
    print(paste0(val,suf))
    a <- Kappa.test(all[[paste0(val)]],all[[paste0(val,suf)]])
    comps[suf,val] <- sprintf(
      "%0.2f %s (%0.2f --- %0.2f)",
      a$Result$estimate,
      ifelse(a$Result$p.value < 0.001,"***",
             ifelse(a$Result$p.value < 0.01,"**",
                    ifelse(a$Result$p.value < 0.05,"*",
                      ifelse(a$Result$p.value < 0.1,".",
                             "")))),
      a$Result$conf.int[1],
      a$Result$conf.int[2]
    )
...
kable(
  comps,
  escape=F,
  caption="Inter and intrarater agreement, Cohen's Kappa (95% confidence interval)", 
  booktab=T, 
  digits=2,
  align="c",
