...
c2o <- function(x) {
  ifelse(x == "D Arg", 1, ifelse(x == "Q Arg", 2, ifelse(x == "D Mod", 3, 4)))
}
...
adata<-droplevels(subset(mdata,simple2lvl != "NEITHER"))
library('ggthemes')
library('extrafont')

adata$reading <- paste("Reading", adata$Reading)
adata$condition2 <- paste(ifelse(adata$Condition_Q,"Q","D"),ifelse(adata$Condition_GP,"Arg","Mod"))
loadfonts(device="pdf")
loadfonts(device="win")

mdata$condition2 <- paste(ifelse(mdata$Condition_Q,"Q","D"),ifelse(mdata$Condition_GP,"Arg","Mod"))
adata$condition2 <- reorder(adata$condition2, c2o(adata$condition2))
...
```{r newdomtab}
domdata <- droplevels(subset(mdata,simple2lvl != "NEITHER"))

domdata$condition2 <- paste(ifelse(domdata$Condition_Q,"Q","D"),ifelse(domdata$Condition_GP,"Arg","Mod"))
domdata$condition2 <- reorder(domdata$condition2, c2o(domdata$condition2))

domdata$dom <- ifelse(domdata$pdom, "PP1 dominant", ifelse(domdata$odom, "OBJ dominant", "Equal strength"))
domdata$dom <- reorder(domdata$dom,ifelse(domdata$dom == "OBJ dominant", 1, ifelse(domdata$dom=="PP1 dominant", 3, 2)))

domdata$Rd <- paste("Reading", domdata$Reading)

domtab.r1 <- with(subset(domdata,Reading==1), table(dom,condition2) %>% prop.table(margin=2) * 100)
domtab.r2 <- with(subset(domdata,Reading==2), table(dom,condition2) %>% prop.table(margin=2) * 100)
domtab <- cbind(domtab.r1,domtab.r2)

domtab %>% kable(digits=1, caption="Proportion of break dominance by condition and reading") %>%
  add_header_above(c(" " = 1, "Reading 1" = 4, "Reading 2" = 4)) %>%
  kable_styling(latex_options = c("hold_position"))
```
```{r domplot}
...
# pp1mod.max <- glmer(PP1~Reading*(Condition_Q+Condition_GP)+(1|IID)+(1|SID),data=mdata,family=binomial)
# ---- best mods ---- 
# pp1mod.noint <- glmer(PP1~Reading+Condition_Q+Condition_GP+(1|IID)+(1|SID),data=mdata,family=binomial)
pp1mod.best <- glmer(PP1~Condition_GP+(1|IID)+(1|SID),data=mdata,family=binomial)
# objmod.rint <- glmer(OBJ~Reading*(Condition_Q+Condition_GP)+(1|IID)+(1|SID),data=mdata,family=binomial)
objmod.best  <- glmer(OBJ~Reading*Condition_GP+(1|IID)+(1|SID),data=mdata,family=binomial)
# ------------------
# objmod.max <- glmer(OBJ~Reading*(Condition_Q*Condition_GP)+(1|IID)+(1|SID),data=mdata,family=binomial)
objmod.rint <- glmer(OBJ~Reading*(Condition_Q+Condition_GP)+(1|IID)+(1|SID),data=mdata,family=binomial)
# objmod.noint <- glmer(OBJ~Reading+Condition_Q+Condition_GP+(1|IID)+(1|SID),data=mdata,family=binomial)
```
...
Table \ref{tab:mns} shows the mean wIRT by experimental condition. The top left cell represents the mean wIRT for the declaritive controls ("D Mod"). The bottom row shows the increase in IRT across the garden path condition. 
...
The difference in the effect of Arg vs. Mod across &plusmn;Q is `r round(cdiff/1000,2)`s. That is, the mean amount that IRT increased for a garden path declarative compared to a non-garden path declarative is `r round(cdiff/1000,2)`s `r ifelse(cdiff > 0, paste("more"),paste("less"))` than the amount that IRT increased for a garden path interrogative compared to a non-garden path interrogative.
...
For the first model, fixed effects of Arg vs. Mod and D vs. Q as well is the interaction between them were included, along with random effects of participant and item. The second model removes the interaction, but keeps both main effects.
...
pdata$dom <- ifelse(pdata$pdom,"PP1",ifelse(pdata$odom, "OBJ", "Neither"))
...
ggplot(pdata,    
  ...
  scale_y_continuous(labels=scales::percent) + 
  scale_fill_brewer(breaks=c("PP1", "Neither", "OBJ"), palette="Greys") +
...
fastTabN <- xtabs(~simple2lvl + condition2, data=fast) %>% 
  as.data.frame() %>% 
  spread(condition2, Freq)  
slowTabN <- xtabs(~simple2lvl + condition2, data=slow) %>% 
  as.data.frame() %>% 
  spread(condition2, Freq)


fastTab <- xtabs(~simple2lvl + condition2, data=fast) %>% 
  prop.table(margin=2) %>% 
  as.data.frame() %>% 
  spread(condition2, Freq)
slowTab <- xtabs(~simple2lvl + condition2, data=slow) %>% 
  prop.table(margin=2) %>% 
  as.data.frame() %>% 
  spread(condition2, Freq)
...
tab %>% kable(
  ...
  col.names=c(" "," ", rep(c("D Mod","Q Mod", "D Arg", "Q Arg"),2)), 
  digits=1
) %>% kable_styling() %>%
