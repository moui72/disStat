...
colnames(objtab)<-cols

pptabTotal<-subset(mdata, !is.na(PP1)) %>%
  group_by(Condition_Q,Condition_GP) %>%
  summarise(n=n()) %>% 
  mutate(Q=ifelse(Condition_Q,"Interrogative","Declarative"))%>%
  mutate(GP=ifelse(Condition_GP,"Garden path","Non-garden path"))%>%
...
colnames(pptab)[1]<-""
pptab[,c(2:3)] <- round(pptab[,c(2:3)]/pptabTotal[,c(2:3)]*100,1)
pptab<-rbind(pptab,c("Both",pp1yGP,pp1nGP))
pptab$Difference <- as.numeric(pptab$`Garden path`) - as.numeric(pptab$`Non-garden path`)
colnames(pptab)<-cols

ctab<-cbind(pptab,objtab)
ctab <- ctab[c(1:4,6:8)]
colnames(ctab)<-c(cols,cols[2:4])
```

The distribution of post-PP1 and post-object breaks split across both conditions can be seen in table \@ref(tab:condtab).

```{r condtab}
kable(
  ctab,
  caption="Percenatge of recordings with PP1/OBJ break by condition"
) %>% 
  add_header_above(c(" "=1,"PP1 Break" = 3,"OBJ Break"=3)) %>% 
...
```{r condobjr2Pre}
r2mdata<-subset(mdata,Reading==2)
cols<-c("","+GP","-GP","Difference")
objtabTotal<-subset(r2mdata, !is.na(OBJ)) %>%
  group_by(Condition_Q,Condition_GP) %>%
  summarise(n=n()) %>% 
  mutate(Q=ifelse(Condition_Q,"Interrogative","Declarative"))%>%
  mutate(GP=ifelse(Condition_GP,"Garden path","Non-garden path"))%>%
...
objtab<-subset(r2mdata, !is.na(OBJ)) %>%
  group_by(OBJ,Condition_Q,Condition_GP) %>%
  summarise(n=n()) %>% 
  subset(OBJ) %>%
  mutate(Q=ifelse(Condition_Q,"Interrogative","Declarative"))%>%
...
pptabTotal<-subset(r2mdata, !is.na(PP1)) %>%
  group_by(Condition_Q,Condition_GP) %>%
  summarise(n=n()) %>% 
  mutate(Q=ifelse(Condition_Q,"Interrogative","Declarative"))%>%
  mutate(GP=ifelse(Condition_GP,"Garden path","Non-garden path"))%>%
...
pptab<-subset(r2mdata, !is.na(PP1)) %>%
  group_by(PP1,Condition_Q,Condition_GP) %>%
  summarise(n=n()) %>% 
  subset(PP1) %>%
  mutate(Q=ifelse(Condition_Q,"Interrogative","Declarative"))%>%
...
The occurence of a PP1 or object break in previewed readings is shown in table \@ref(tab:condpp1r2).

```{r condpp1r2}
kable(
  ctab,
  caption="Percentage of previewed recordings with a PP1/OBJ break, by condition"
) %>% 
  add_header_above(c(" "=1,"PP1 Break" = 3,"OBJ Break"=3)) %>% 
  kable_styling(latex_options = "hold_position")
```
