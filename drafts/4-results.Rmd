...
Models predicting the PP1 break and the object break are presented in table \ref{tab:lmPros}. No random slopes were included, as models with random slopes did not converge.
...
df %>%  
  arrange(df$r1delbyp) %>% 
  kable(
    col.names = c("Participant", "Reading 1 delay category","Mean reading 1 delay"), 
    caption="Mean reading 1 delay by participant (seconds)", 
    digits = 2
  ) %>% kable_styling(latex_options = c("hold_position", "repeat_header"))%>% 
  footnote(catdes)
```
...
kable(
  cat,
  caption="Percentage of reading 2 recordings containing PP1/OBJ break by condition, slow reading 1 delay only",
...
I'm thinking I will do a this breakdown for the break patterns, rather than the individual breaks. I might also do a similar breakdown where I categorize R1 delay for every recording, rather than by subject means.

Do you think there are other fruitful avenues to pursue?
```

\clearpage

## Demographic and self-reported reading habits' effect on the data

Some basic demographic data and self reported reading habits are available by way of a questionairre admistered after data collection. The prosody data and the IRT data can be considered through this lens.

``` 
TODO

Tables for the above. Maybe models if anything seems interesting.

Question for Di: If nothing is interesting, omit this?
