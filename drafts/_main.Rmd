...
This section reports various analyses of the recordings obtained. There is a total of 32 participants included, 48 total items, 16 of which are experimental items, with each item read twice by each participant. There were 4 versions of the experiment. Some items are missing due to technical issues during data collection. Other data were excluded due to factors explained later. Ultimately, `r nrow(mdata)` recordings are analyzed (`r nrow(irt_data)` pairs).
...
del.irt.full <- lmerTest::lmer(
  wirt.log10 ~  Condition_GP + (1 | Participant) + 
    (1 | Item),
  data = irt_data,
  REML=F
)
