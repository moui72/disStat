...
This section will review the questions motivating this study and discuss the extent to which those questions are answered, or not. It will then go on to develop further questions, and propose further studies to explore those new questions and the ones left unanswered here. Finally, it will summraize the findings and the current standing of this area of research.
...
Ultimately the data here are unable to provide a clear answer to whether or not IRT represents a behavioral correlate of the intuition that PP-attachment garden paths are rendered easier to parse when presented as polar questions. Mixed-effect regression analyses were not able to detect stastical significance for the interaction between Speech Act and PP2 Status. This does not, of course, negate the intuition; it simply means that we have not yet found a behavior that can be said with certainty to correspond to that intuition.

There are a number of issues that might be preventing the observation of behavior that confirms the intuition, and there are also possible explanations for why the intuition might exist without a behavioral correlate available at all. 

The experimental paradigm used here may have been too lenient to capture the behavior it is looking for: it relies on participants to begin Reading 1 without any lookahead simply by their own recognizance rather than any experimenter-enforced mechanism. Perhaps some participants were able to, perhaps unintentionally, get enough information a priori to negate the effect of the interaction on their IRT.

IRT may be the wrong measure for detecting a behavioral correlate of the intuition: Reading 1 may give tip Reading 2's hand, in a sense. If Reading 1 is preparing the reader to the extent that a processing struggle is not a part of what IRT represents, then the behavioral correlate of the intuition would need to be measured on Reading 1 somehow. This would require an alternative explanation for the significant effect of PP2 status on Reading 2, though.

It's also possible that the added complexity of interrogativity is not actually making the reanalysis of Arg sentences easier at all, but creating the illusion of such, by distracting the reader with a different sort of difficulty. If you try to solve a puzzle in a dark room, you might not realize how difficult the puzzle is, because you're so distracted by the lack of light. Perhaps we are being distracted by the processing cost of interrogativty, and thereby being less sensitive to the cost of reanalysis.
...
For the interrogatives, though, this contrast is obscured by the need to apply a rising prosodic contour over the final nuclear accent in the sentence: i.e. PP2, resulting in not a change in syntactic branching direction, but the anticipation of a tonal change, which might sound very similar to a prosodic break. The symbol "//" indicates the start of the rising contour.
...
The critical issue here is that in (@qhi), the absence of a syntactically-motivated break is hidden by the juncture created by the start of the final-rising contour of a question two words after the position of the break. It's not impossible that the need to make the mechanical preparation necessary to execute a rising contour could result in a hesitation or pause at the preceding syntactic juncture most welcoming to it, the PP1 break position. 
...
While IRT did not detect a behavioral correlate of the motivating intuition for this study, it did show a robust effect of PP2 Status, indicating that it is a useful diagnostic for parsing difficulty. The IRT data reported also supported prior findings that interrogatives are more difficult to parse than similar declarative findings.

That reading (Reading 1 vs. Reading 2) frequently had an effect on prosodic phrasing also supports the finding of @fodor2019center that item preview should be tightly controlled when one is studying prosodic structure.
